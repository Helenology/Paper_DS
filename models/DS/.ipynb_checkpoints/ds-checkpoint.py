...
class DS:
    def __init__(self, p, q, train_list):
        ...
        :return:
            None. Builds an internal CD baseline model from `train_list` for reuse.
        """
        self.p = p                            # image height
        self.q = q                            # image width
        self.N = len(train_list)              # number of training images
        self.train_list = train_list          # training image paths
        self.CD_model = CD(p, q, train_list)  # an internal CD model

    def new_location_filter(self, bandwidth, size=3, middle_point=(1, 1)):
        """
        Build a spatial location-aware smoothing filter centered at `middle_point`.

        :param bandwidth: bandwidth used inside K_tf to turn normalized offsets into weights.
        :param size: filter window size (size × size). Default: 3.
        :param middle_point: (row, col) index (0-based) of the center inside the size×size window.
                             Offsets are normalized by (p, q) to be scale-free. Default: (1, 1).

        :return:
            location_filter: tf.Tensor with shape [size, size, 1, 1], nonnegative and L1-normalized,
                             suitable for depthwise 2D convolution.
        """
